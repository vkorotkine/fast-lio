...
void Preprocess::oust32_row_col_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  pcl::PointCloud<ouster_ros_row_col::Point> pl_orig;
  pcl::fromROSMsg(*msg, pl_orig);
  int plsize = pl_orig.size();
  pl_corn.reserve(plsize);
  pl_surf.reserve(plsize);
  if (feature_enabled)
  {
    ...
    for (uint i = 0; i < plsize; i++)
    {
      ...
      // added_pt.curvature = pl_orig.points[i].t * time_unit_scale;
      // double t = (double)pl_orig.points[i].colIdx / 2048.0 * 100.0 / 1e3;
      double t = (double)pl_orig.points[i].colIdx / 2048.0 * 100.0 * time_unit_scale;
      added_pt.curvature = t;

      if (pl_orig.points[i].rowIdx < N_SCANS)
      {
        pl_buff[pl_orig.points[i].rowIdx].push_back(added_pt);
      }
    }
